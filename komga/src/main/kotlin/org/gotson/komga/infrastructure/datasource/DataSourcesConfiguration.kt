...
class DataSourcesConfiguration(
  ...
  fun sqliteDataSourceRW(): DataSource =
    if (komgaProperties.database.isPostgresql())
      buildPostgresDataSource("PostgresMainPool", komgaProperties.database)
    else
      buildSqliteDataSource("SqliteMainPoolRW", SqliteUdfDataSource::class.java, komgaProperties.database)
        .apply {
  ...
  fun sqliteDataSourceRO(): DataSource =
    if (komgaProperties.database.isPostgresql())
      sqliteDataSourceRW()
    else if (komgaProperties.database.shouldSeparateReadFromWrites())
      buildSqliteDataSource("SqliteMainPoolRO", SqliteUdfDataSource::class.java, komgaProperties.database)
    else
      sqliteDataSourceRW()

  @Bean("tasksDataSourceRW")
  fun tasksDataSourceRW(): DataSource =
    if (komgaProperties.tasksDb.isPostgresql())
      buildPostgresDataSource("PostgresTasksPool", komgaProperties.tasksDb)
    else
      buildSqliteDataSource("SqliteTasksPoolRW", SQLiteDataSource::class.java, komgaProperties.tasksDb)
        .apply {
  ...
  fun tasksDataSourceRO(): DataSource =
    if (komgaProperties.tasksDb.isPostgresql())
      tasksDataSourceRW()
    else if (komgaProperties.tasksDb.shouldSeparateReadFromWrites())
      buildSqliteDataSource("SqliteTasksPoolRO", SQLiteDataSource::class.java, komgaProperties.tasksDb)
    else
      tasksDataSourceRW()
  ...
  private fun buildPostgresDataSource(
    ...
    return HikariDataSource(
      HikariConfig().apply {
        ...
        this.jdbcUrl = databaseProps.url
        this.username = databaseProps.username
        this.password = databaseProps.password
